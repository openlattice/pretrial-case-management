  ...
  switch (action.type) {
    ...
    case replaceEntity.case(action.type): {
      return replaceEntity.reducer(state, action, {
        ...
        FAILURE: () => state.set(SUBMIT.REPLACING_ENTITY, false).set(SUBMIT.ERROR, action.value),
    ...
    case replaceAssociation.case(action.type): {
      return replaceAssociation.reducer(state, action, {
        REQUEST: () => state
          ...
          .set(SUBMIT.ERROR, ''),
        SUCCESS: () => state.set(SUBMIT.REPLACING_ASSOCIATION, true).set(SUBMIT.ERROR, ''),
        FAILURE: () => state.set(SUBMIT.REPLACING_ASSOCIATION, false).set(SUBMIT.ERROR, action.value),
